var GTD_CONTROL_TXTBOX = 0;
var GTD_CONTROL_TXTAREA = 1;
var GTD_CONTROL_SELECT = 2;
var GTD_CONTROL_MULTISELECT = 3;
var GTD_CONTROL_YESNO = 4;
var GTD_CONTROL_CHECKBOX = 5;
var GTD_CONTROL_RADIOBOX = 6;
var GTD_CONTROL_DATETIME = 7;
var GTD_CONTROL_FILE = 8;
...
function gtdRoleCustOnClick(frmName, roleName, roleParam, joinChr, className)
{
    if (joinChr.length ==0) {
        joinChr = '&amp;';
    }
...
function gtdPortOnChange(srvtype, portfld)
{
    fld = gE(portfld);
    switch(srvtype) {
    case 'POP3':
...
function gtdDOMAddEvent(obj, evType, fn, useCapture){
...
function gtdDOMRemoveEvent(obj, evType, fn, useCapture)
{
...
function gtdFillStaffSelect(sel, values)
{
    if (values.length > 0) {
        var val = sel.value;
        var uid = '';
...
function gtdFillSelect(sel, values)
{
    if (values.length > 0) {
        var val = sel.value;
        var myKey = '';
...
function gtdFillCustomFlds(tbl, result, before)
{
    var flds = gtdGetElementsByClassName(tbl, 'custfld_value');
    var values = Array();
    ...
    if(result.length > 0) {       
        for (n = 0; n < result.length; n++) {
            ...
            if (data != null) {
                ...
                inp = gtdCreateInpField(controlType, fieldName, defaultValue, currentValue, fieldLength, values);
                tbl.insertBefore(gtdCreateCustFldRow(name, desc, inp), before);
            }
...
function gtdGetElementsByClassName(parentEle, className)
{
    var ele = Array();
    if (parentEle.hasChildNodes()) {
        ...
        for (var n = 0; n < chlds.length; n++) {
            ...
            if (chlds[n].hasChildNodes()) {
                ele.concat(gtdGetElementsByClassName(chlds[n], className));
            }
...
function gtdCreateInpField(controlType, fieldName, defaultValue, currentValue, fieldLength, values)
{   
    ...
    {
        case '0':
            ele = gtdCreateTextBox(fieldName, value, length, fieldLength);
            break;
        case '8':
            ele = gtdCreateFile(fieldName, length);
            break;
        case '1':
            ele = gtdCreateTextArea(fieldName, value, length, 5);
            break;
        case '2':
        case '3':
            ele = gtdCreateSelect(fieldName, values, value, 1, false);
            break;
        case '5':
            ele = gtdCreateCheckbox(fieldName, values, value);
            break;
        case '6':
        case '4':
            ele = gtdCreateRadiobox(fieldName, values, value);
            break;
        case '7':
            ele = gtdCreateDateTime(fieldName, value);
            break;
    }
    return ele; 
}

function gtdCreateTextArea(fieldName, value, cols, rows)
{
...
function gtdCreateTextBox(fieldName, value, length, maxLength)
{
...
function gtdCreateFile(fieldName, length)
{
    var l = cE(document.all?'<input name="'+fieldName+'">':'input');
...
function gtdCreateSelect(fieldName, values, value, size, multiple)
{
...
function gtdCreateCheckbox(fieldName, values, value)
{
    var ele = Array();
    var l = null;
    for (var n=0; n<values.length; n++) {
        ...
        l.value = values[n][0];
        if (isArray(value)) {
            l.checked = gtdInArray(l.value, value);
        } else {
...
function gtdCreateRadiobox(fieldName, values, value)
{
    var ele = Array();
    var l = null;
    for (var n=0; n<values.length; n++) {
...
function gtdCreateDateTime(fieldName, value)
{    
    var l = gtdCreateTextBox(fieldName, value, 50, 25);
    return l;
}

function gtdCreateCustFldRow(name, description, ele)
{
    var tr = cE('tr');
    var td1 = cE('td');
    var td2 = cE('td');
...
function gtdInArray(needle, haystack)
{
