...
Class gtdBaseObjectHandler extends XoopsObjectHandler {
    /**
     * Database connection
     * 
     * @var	object
   ...
   /**
    * create a new  object
    * @return object {@link gtdBaseObject}
    * @access public
    */
    function &create()
    {
    ...
    */
	...
	function update_observations_paiement(&$obj)
	{
		global $xoopsLogger;
		 if (!$obj->cleanVars()) {
			$xoopsLogger->addExtra('gtdBaseObjectHandler.php', "update_observations_paiement cleanVars= FALSE");
			return false;
		}
		$sql = $this->_updateObservationsPaiementQuery($obj);
		$xoopsLogger->addExtra('gtdBaseObjectHandler.php', "requete UPDATE OBS = $sql");
		$result = $this->_db->query($sql);
        if (!$result) {
			$xoopsLogger->addExtra('gtdBaseObjectHandler.php', 'Appel de Retour en erreur de l\execution de la requete UPDATE OBS !!!!');
            return false;
        }
		return true;
	}
    ...
    {
		global $xoopsLogger;
		$xoopsLogger->addExtra('gtdBaseObjectHandler.php', 'entree dans  insert');
        // Make sure object is of correct type
        if (strcasecmp($this->classname, get_class($obj)) != 0) {
			$xoopsLogger->addExtra('gtdBaseObjectHandler.php', 'Vérif classname FALSE');
            return false;
        }
        
        // Make sure object needs to be stored in DB
        if (!$obj->isDirty()) {
			$xoopsLogger->addExtra('gtdBaseObjectHandler.php', 'Vérif isDirty TRUE');
            return true;
        }
        
        // Make sure object fields are filled with valid values
        if (!$obj->cleanVars()) {
			$xoopsLogger->addExtra('gtdBaseObjectHandler.php', 'cleanVars return FALSE');
            return false;
        }
        ...
        if ($obj->isNew()) {
			...
			$xoopsLogger->addExtra('gtdBaseObjectHandler.php', 'Appel de _insertQuery');
        ...
        if (!$result) {
			$xoopsLogger->addExtra('gtdBaseObjectHandler.php', 'Appel de Retour en erreur de l\execution de la requete INSERT ou UPDATE !!!!');
            return false;
        }
