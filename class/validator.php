    ...
    */
    function validate() 
    {
        ...
        if(!preg_match($pattern,$this->email)){
            $this->setError(_GTD_MESSAGE_INVALID);
        }
        if (strlen($this->email)>100){
            $this->setError(_GTD_MESSAGE_LONG);
        }
        if (strlen($this->email)<5){
            $this->setError(_GTD_MESSAGE_SHORT);
        }
    }
}
    ...
    */
    function validate() 
    {
        if (!is_numeric($this->text) && (strlen($this->text) > 0 && !$this->forceentry)) {
            $this->setError(_GTD_MESSAGE_NOT_NUMERIC);
        }
    }   
}
    ...
    */
    function validate() 
    {
    	...
    	if ( strlen($this->uname) > $xoopsConfigUser['maxuname'] ) {
    		$this->setError(sprintf(_GTD_MESSAGE_LONG, $xoopsConfigUser['maxuname']));
    	}
    	if ( strlen($this->uname) < $xoopsConfigUser['minuname'] ) {
    		$this->setError(sprintf(_GTD_MESSAGE_SHORT, $xoopsConfigUser['minuname']));
    	}
    	foreach ($xoopsConfigUser['bad_unames'] as $bu) {
    		if ( !empty($bu) && preg_match("/".$bu."/i", $this->uname) ) {
    			$this->setError(_GTD_MESSAGE_RESERVED);
    			break;
    		}
    	}
    	if ( strrpos($this->uname,' ') > 0 ) {
    	    $this->setError(_GTD_MESSAGE_NO_SPACES);
    	}
    	$sql = "SELECT COUNT(*) FROM ".$xoopsDB->prefix('users')." WHERE uname='".addslashes($this->uname)."'";
    	$result = $xoopsDB->query($sql);
    	list($count) = $xoopsDB->fetchRow($result);
    	if ( $count > 0 ) {
    		$this->setError(_GTD_MESSAGE_UNAME_TAKEN);
    	}
    }
}
    ...
    */
    function validate() 
    {
        ...
        if ( !isset($this->pass) || $this->pass == '' || !isset($this->vpass) || $this->vpass == '' ) {
    		$this->setError(_GTD_MESSAGE_NOT_SUPPLIED);
    		//$stop .= _US_ENTERPWD.'<br />';
    	}
    	if ( (isset($this->pass)) && ($this->pass != $this->vpass) ) {
    		$this->setError(_GTD_MESSAGE_NOT_SAME);
    		//$stop .= _US_PASSNOTSAME.'<br />';
    	} elseif ( ($this->pass != '') && (strlen($this->pass) < $xoopsConfigUser['minpass']) ) {
    		$this->setError(sprintf(_GTD_MESSAGE_SHORT, $xoopsConfigUser['minpass']));
        ...
        if($this->required == 1){                                // If value is required
            if(is_array($this->checkText) && isset($this->checkText['size'])) {     // If this is a file
                if($this->checkText['name'] == ""){
                    $this->setError(_GTD_MESSAGE_REQUIRED);           // Return message saying required value
                }
            } else {                                                    // If not a file
                if($this->pattern != ""){                               // If regex pattern is not empty
                    if(!preg_match("/".$this->pattern."/", $this->checkText)){  // Check regex against supplied text
                        $this->setError(_GTD_MESSAGE_INVALID);        // Return message saying invalid value 
                    }
                } else {
