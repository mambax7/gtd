...
class gtdTicketFieldDepartmentHandler
{
    var $_db;
    var $_hField;
    var $_hDept;
     ...
     * @access public
     */
    function gtdTicketFieldDepartmentHandler(&$db)
    {
        $this->_db =& $db;
        $this->_hField =& gtdGetHandler('ticketField');
    ...
    function departmentsByField($field, $id_as_key = false)
    {
        ...
        $sql   = sprintf('SELECT d.* FROM %s d INNER JOIN %s j ON d.id = j.deptid WHERE j.fieldid = %u',
            $this->_db->prefix('gtd_departments'), $this->_db->prefix('gtd_ticket_field_departments'), $field);
     ...
     * @param bool $id_as_key Should object ID be used as array key?
     * @return array array of {@Link gtdTicketField} objects
     * @access public
     */
    function fieldsByDepartment($dept, $id_as_key = false)
    {
        ...
        $sql   = sprintf('SELECT f.* FROM %s f INNER JOIN %s j ON f.id = j.fieldid WHERE j.deptid = %u ORDER BY f.weight',
            $this->_db->prefix('gtd_ticket_fields'), $this->_db->prefix('gtd_ticket_field_departments'), $dept);
        $ret = $this->_db->query($sql);
        $arr = array();
    ...
    *
    * @param mixed $staff single or array of uids or {@link gtdTicketField} objects
    ...
    */        
     ...
     *
     * @param mixed $field single or array of field ids or {@link gtdTicketField} objects
     * @param int $deptid Department ID
     * @return bool True if successful, False if not
     * @access public
     */
    ...
    {
        $sql = 'DELETE FROM '.$this->_db->prefix('gtd_ticket_field_departments');
     ...
     *
     * @param mixed $field fieldid or {@Link gtdTicketField} object
     * @param mixed $dept deptid or {@Link gtdDepartment} object
     * @return bool True if Successful, False if not
     * @access private
     */
    ...
    function _addJoinerRecord($fieldid, $deptid)
    {
        $sql = sprintf('INSERT INTO %s (fieldid, deptid) VALUES (%u, %u)', 
            $this->_db->prefix('gtd_ticket_field_departments'), $fieldid, $deptid);
        $ret = $this->_db->query($sql);
        return $ret;
    }
