<?php
//$Id: ticketField.php,v 1.11 2005/09/07 14:56:56 eric_juden Exp $
//  ------------------------------------------------------------------------ //
...
/**
 * gtdTicketField class
 *
 * Metadata that represents a custom field created for gtd
 *
 * @author Brian Wahoff <ackbarr@xoops.org>
 * @access public
 * @package gtd
 */
class gtdTicketField extends XoopsObject
{
     ...
     * @access public
     */
    function gtdTicketField($id = null)
    {
        ...
        $this->initVar('controltype', XOBJ_DTYPE_INT,  GTD_CONTROL_TXTBOX, true);
...
class gtdTicketFieldHandler extends gtdBaseObjectHandler
{
    /**
     * Name of child class
     * 
	 ...
	 * @access 	private
	 */
	var $_dbtable = 'gtd_ticket_fields';
	var $id = 'id';
	 ...
	 */
	function gtdTicketFieldHandler(&$db) 
	{
		parent::init($db);
    } 
	...
	function insert(&$obj, $force = false)
	{
        ...
        if ($ret = parent::insert($obj, $force)) {
            ...
            if ($add_field) {
                gtdAddDBField('gtd_ticket_values', $fieldname, $mysql['fieldtype'], $mysql['length']);
            } elseif ($alter_table) {
                gtdRenameDBField('gtd_ticket_values', $old_name, $new_name, $mysql['fieldtype'], $mysql['length']);
            }
        }
        return $ret;
	...
	function delete($obj, $force=false)
	{
        ...
        if (!$ret = gtdRemoveDBField('gtd_ticket_values', $obj->getVar('fieldname'))) {
    ...
    {
        $hFieldDept =& gtdGetHandler('ticketFieldDepartment', 'gtd');
