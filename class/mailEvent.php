 ...
 * @package gtd
 */ 
class gtdMailEvent extends XoopsObject {
    function gtdMailEvent($id = null) 
	{
...
/**
 * gtdMailEventHandler class
 *
 * MailEvent Handler for gtdMailEvent class
 *
 * @author Eric Juden <ericj@epcusa.com> &
 * @access public
 * @package gtd
...
class gtdMailEventHandler extends gtdBaseObjectHandler {
	/**
     * Name of child class
     * 
     * @var	string
     ...
     */	
	function gtdMailEventHandler(&$db)
	{
	    parent::init($db);
    }
    ...
    function _selectQuery($criteria = null, $join = false)
    {
        if(!$join){
	        ...
	        $sql = sprintf("SELECT e.* FROM %s e INNER JOIN %s d ON d.id = e.mbox_id", $this->_db->prefix('gtd_mailevent'), $this->_db->prefix('gtd_department_mailbox')); 
	    }
	 ...
	 * @return array array of {@link gtdMailEvent} objects
	 * @access	public	
	 */	
