...
class gtdEmailStore
{
    ...
    function gtdEmailStore()
    {
        $this->_hResponse  =& gtdGetHandler('responses');
        $this->_hTicket    =& gtdGetHandler('ticket');
        $this->_hMailEvent =& gtdGetHandler('mailEvent');
        $this->_errors     = array();
    }
     ...
     * @access public
     * @param object $msg {@link gtdParsedMsg} object Message to add
     * @param object $user {@link xoopsUser} object User that submitted message
     * @param object $mbox {@link gtdDepartmentMailBox} object. Originating Mailbox for message
     * @return mixed Returns {@link gtdTicket} object if new ticket, {@link gtdResponses} object if a response, and false if unable to save.
     */
    function &storeMsg(&$msg, &$user, &$mbox, &$errors)
    {
        ...
        case _GTD_MSGTYPE_RESPONSE:
            if (!$ticket = $this->_hTicket->getTicketByHash($msg->getHash())) {
                $this->_setError(_GTD_RESPONSE_NO_TICKET);
                return false;
            }
            
            
            if ($msg->getEmail() != $ticket->getVar('email')) {
                $this->_setError(sprintf(_GTD_MISMATCH_EMAIL, $msg->getEmail(), $ticket->getVar('email')));
                return false;
            }            
    ...
    function _saveAttachments($msg, $ticketid, $responseid = 0) 
    {
        ...
        if($xoopsModuleConfig['gtd_allowUpload']){
            ...
            foreach ($attachments as $attach) {
                ...
                $validators[] = new ValidateFileSize($dir.$fname, $xoopsModuleConfig['gtd_uploadSize']);
                $validators[] = new ValidateImageSize($dir.$fname, $xoopsModuleConfig['gtd_uploadWidth'], $xoopsModuleConfig['gtd_uploadHeight']);
                
                if (!gtdCheckRules($validators, $errors)) {
                    //Remove the file
            ...
            $this->_setError(_GTD_MESSAGE_UPLOAD_ALLOWED_ERR);   // Error: file uploading is disabled
        }
    }
    ...
    function _addAttachmentError($errors, $msg, $fname)
    {
        if($errors <> 0){
            ...
            $this->_setError(sprintf(_GTD_MESSAGE_UPLOAD_ERR, $fname, $msg->getEmail(), $error));
        }                
    }        
}
?>