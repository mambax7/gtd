...
function list_blocks()
{
	...
	<form action='admin.php' name='blockadmin' method='post'>
		...
		<tr valign='middle'>
			<th>"._AM_TITLE."</th>
			<th align='center' nowrap='nowrap'>"._AM_GTD_POSITION."</th>
	...
	foreach( array_keys( $block_arr ) as $i ) {
		...
		foreach( $module_list as $mid => $mname ) {
			if( in_array( $mid , $selected_mids ) ) {
				$module_options .= "<option value='$mid' selected='selected'>" . $gtds->displayTarea($mname) . "</option>\n" ;
			} else {
				$module_options .= "<option value='$mid'>" . $gtds->displayTarea($mname) . "</option>\n" ;
			}
		}
...
function list_groups()
{
	...
	$form->addAppendix('module_admin',$xoopsModule->mid(),$gtds->displayTarea($xoopsModule->name()).' '._AM_ACTIVERIGHTS);
	$form->addAppendix('module_read',$xoopsModule->mid(),$gtds->displayTarea($xoopsModule->name()).' '._AM_ACCESSRIGHTS);
	foreach( $item_list as $item_id => $item_name) {
		$form->addItem( $item_id , $gtds->displayTarea($item_name) ) ;
	}
	echo $form->render() ;
    echo "</div>";	
}
